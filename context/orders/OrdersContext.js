import shopApi from '../../src/firebase/config';
...
const addOrder = dispatch => async (cartItems, totalAmount, userId) => {
  const date = new Date();
  try {
    const response =  await  firebase.firestore().collection('orders').add({
    cart: cartItems,
...
const getOrders = dispatch => async userId => {
  try {
    const response = await firebase.firestore().collection.where("user", "==", {userId}).get();
    const orders = [];
