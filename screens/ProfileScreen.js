...
const ProfScreen = props => {
    ...
    return (
      ...
      <SharedElement id={`item.${hotel.id}.photo`}>
        <Image source={{uri: hotel.image}} style={{width: '100%', height: height - 450, borderBottomLeftRadius: 10, borderBottomRightRadius: 10}} resizeMode="cover"/>
      </SharedElement>

      <View style={{flexDirection: 'row',alignItems: 'center',position: 'absolute', bottom: 14, left: 10}}>

      <SharedElement id={`item.${hotel.id}.profilePic`}>
        <Image
        source={{uri: hotel.profilePic}} 
    ...
    <View style={{flex: 1, flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', paddingRight: 20}}>
      ...
      <View>
      <SharedElement id={`item.${hotel.id}.username`}>
      <Text style={{color: 'white', fontSize: 16, fontWeight: 'bold'}}>{hotel.username}</Text>
      </SharedElement>
      <SharedElement id={`item.${hotel.id}.readtime`}>
        <Text style={{color: 'white', fontSize: 14,}}>{hotel.readtime} read</Text>
      </SharedElement>
      </View>
     ...
     <SharedElement id={`item.${hotel.id}.text`} style={{ width: width - 30, marginBottom: 14}}>
        <Text style={{color: 'black', fontSize: 22,fontWeight: 'bold', lineHeight: 32}}>{hotel.title}</Text>
     </SharedElement>
